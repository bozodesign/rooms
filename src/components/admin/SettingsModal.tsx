...
export default function SettingsModal({ isOpen, onClose, lineUserId }: SettingsModalProps) {
  ...
  const { data: settingsData, mutate } = useSWR(
    isOpen ? ['/api/admin/settings', lineUserId] : null,
  ...
  const handleOpenPromptPay = () => {
    setCurrentView('promptpay')
    setDragCurrentY(0)
  }
  ...
  return (
    <>
      ...
      >
        {/* Drag Handle */}
        <div
          className="sticky top-0 bg-gradient-to-r from-gray-700 to-gray-800 text-white p-4 rounded-t-3xl cursor-grab active:cursor-grabbing"
